...
def compress_map_data(namespace: str):
    ...
    map_files = sorted(
        [
            ...
            if f.startswith("map_data_") and f.endswith(".txt")
        ]
    )
    ...
    for map_file in map_files:
        ...
        with open(
            os.path.join(dataset_path, f"map_metadata_{timestamp}.json"), "r"
        ) as f:
            metadata = f.read()
        ...
        with open(
            os.path.join(compressed_path, f"compressed_map_{timestamp}.pkl"), "wb"
        ) as f:
            pkl.dump(compressed_data, f)

        print(
            f"Compressed map data saved for timestamp {timestamp} in namespace '{namespace}'."
        )
